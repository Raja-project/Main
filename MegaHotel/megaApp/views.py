...
def hotelDetail(request,hid1=None):
     ...
     if 'em' in request.session:
          if request.method=='POST':
               ...
               pro.save()
                #email function
               subject = "Mega Hotels Online Room Booking System"
               message = "Your Room has been Booked successfully""Your Hotel will be : "+ob1.hname+" Payble Amount : "+str(fp)
               sender  = "ojd4222@gmail.com"
               recipient_list=[]

               print(recipient_list.append(request.session["em"] )) #Replace with the recipient's email address
               print(subject,message,sender,recipient_list)

               send_mail(subject, message, sender, recipient_list,fail_silently=False)
               print("email sent successfully")
               return render (request,'megaTemp/hotel-details.html',{'hd':ob1})


     else:
          
      return render (request,'megaTemp/msg.html')     
